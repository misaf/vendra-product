...
- Keep API Platform serialization and API routes in `misaf/vendra-product-api`.
- `Services\ProductPurchaseQuoter::quote()` owns purchase eligibility and catalog pricing (active category, stock, the newest price in the currency, since prices are append-only history) and returns a `Data\ProductPurchaseQuote` or an `Enums\ProductPurchaseRefusalEnum` case per request, loading products in one query. Checkout callers reuse it rather than re-deriving the rules; keep cart, order, and API types out of it.
- Take stock with `Actions\DeductProductStockAction` (locks products in id order, rechecks under the lock, all or nothing, throws `Exceptions\InsufficientProductStockException`, switches `in_stock` off with the last unit) and return it with `Actions\RestockProductsAction`. Never decrement `quantity` inline.
- Duplicate products only through `Actions\DuplicateProductAction`, which copies the record with unique copy names and slugs, prices, media, attribute selections, and tags in one transaction. `DuplicateProductTableAction` delegates to it; do not rebuild the copy in Filament hooks.
