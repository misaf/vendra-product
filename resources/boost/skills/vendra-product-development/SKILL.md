...
- Add or update focused Pest coverage, then run `php artisan test --compact --testsuite=vendra-product` and `composer stan`.
...
- Purchase eligibility and catalog pricing live in `Services\ProductPurchaseQuoter`, which batches its product query and returns typed `ProductPurchaseQuote` results or `ProductPurchaseRefusalEnum` cases. It charges the newest price row in the currency; prices are append-only history. It must not import cart, order, or API types.
...
- Product duplication lives entirely in `Actions\DuplicateProductAction` (unique copy names and slugs per tenant, prices, media, attribute selections, tags); the Filament table action only delegates to it.
...
- Run checks from the host app: `php artisan test --compact --testsuite=vendra-product` and `composer stan`.
